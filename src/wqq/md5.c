...
static void MD5Update(MD5_CTX * context, unsigned char const *buf,
					  unsigned len);
...
char *lutil_md5_data(const unsigned char *data, unsigned int len,
					 char *buf)
{
	MD5_CTX ctx;
...
char *lutil_md5_digest(const unsigned char *data, unsigned int len,
					   char *buf)
{
	MD5_CTX ctx;
